...
export function signPsbtWithRandomWIF({ inputs, outputs, addressType, network, autoFinalized }: SignPsbtWithRandomWIFOptions) {
...
export class Wallet {
  ...
  private readonly _addressType: AddressType;
  ...
  signPsbt(psbt: bitcoin.Psbt, opts?: SignOptions) {
    ...
    psbt.data.inputs.forEach((v, index) => {
      ...
      if (isNotSigned && isP2TR && lostInternalPubkey) {
        const tapInternalKey = toXOnly(this.pubkey);
...
export const DODPROTOCOL_ENVELOPE_ID = 'dod';
...
export function getDodRevealScript(childNodeXOnlyPubkey: Buffer, opType: number[], payload: Buffer) {
  let ops = `${childNodeXOnlyPubkey.toString('hex')} OP_CHECKSIG OP_0 OP_IF `;
  ops += `${Buffer.from(DODPROTOCOL_ENVELOPE_ID, 'utf8').toString('hex')}`;
  ops += ` ${Buffer.from(Array.from(opType)).toString('hex')}`;
  const chunks = chunkBuffer(payload, 520);
  for (let chunk of chunks) {
    ops += ` ${chunk.toString('hex')}`;
  }
...
export const prepareDodScripts = (
  ...
  const opcode = [getOpcode(opType)];
  const revealScript =
    lock_time == undefined
      ? getDodRevealScript(childNodeXOnlyPubkey, opcode, payload)
      : getDodRevealScriptForStaking(lock_time, childNodeXOnlyPubkey, opcode, payload);
  const hashscript = script.fromASM(revealScript);
  const scriptTree = {
    output: hashscript,
  };
...
export function getDodRevealScriptForStaking(lockTime: number, childNodeXOnlyPubkey: Buffer, opType: number[], payload: Buffer) {
  let ops = `${bitcoin.script.number.encode(lockTime).toString('hex')} OP_CHECKLOCKTIMEVERIFY OP_DROP ${childNodeXOnlyPubkey.toString(
    'hex',
...
export type OpType = 'init' | 'mint' | 'tele' | 'burn' | 'call' | 'send' | 'dele' | 'stak' | 'mine';

export function getOpcode(opType: OpType) {
...
export function calcRevealFee(feeRate: number, inputNum: number, outputs: Output[], hashLockP2TROutputLen = 0): number {
...
export function prepareCommitAndRevealTx({
  ...
  const { hashLockP2TR } = prepareCommitRevealConfig(fundingKeypair.childNodeXOnlyPubkey, payload, network);
...
export function prepareDodCommitAndRevealTx({
  wif,
  opType,
  feeRate,
  utxos,
  ...
  const _wif = wif ?? ECPair.makeRandom({ network });
  const fundingKeypair = getKeypairInfo(_wif, network);
  const { hashLockP2TR } = prepareDodScripts(fundingKeypair.childNodeXOnlyPubkey, opType!, payload, network);
  const hashLockP2TROutputLen = hashLockP2TR.redeem!.output!.length;
  const revealTxFee = calcRevealFee(feeRate, revealInputs, revealOutputs, hashLockP2TROutputLen);
  const outputValue = revealOutputs.reduce((a, b) => a + b.value, 0);
  const revealNeed = revealTxFee + outputValue + 500;
  // reveal output
  let outputsLen = 1;
  let value = 0;
  const inputs: UTXO[] = [];
...
export function edictEncoder() {
  let test_bytes = IDL.encode([IDL.Vec(IDL.Nat32), IDL.Opt(IDL.Nat), IDL.Nat32], [Array.from([0]), [BigInt(1000)], 0]);
  console.log(Buffer.from(test_bytes).toString('hex'));
}

export interface EdictInterface {
  skip: number[];
  amount?: bigint;
  output: number;
}

export class RainbowEdict {
  constructor(public payload: EdictInterface) {}

  static _encode(payload: EdictInterface) {
    return IDL.encode(
      [IDL.Vec(IDL.Nat32), IDL.Opt(IDL.Nat), IDL.Nat32],
      [Array.from(payload.skip), payload.amount === undefined ? [] : [payload.amount], payload.output],
    );
  }

  static encode(payload: EdictInterface) {
    return Array.from(Buffer.from(RainbowEdict._encode(payload)));
  }
}
